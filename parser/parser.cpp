...
void parser::init(Manufacturers manufacturer, std::string_view machine) {
  ...
  auto topLayer = std::unique_ptr<definition_base>(
      MachineDefinitions::getDefinition(manufacturer, machine));
  if (topLayer) {
...
void parser::match(int code) {
  ...
  if (next != Token::done) {
    next = m_lexer->next();
    if (next == Token::error || next == Token::unknown_function ||
        next == Token::unknown_code) {
...
std::vector<parse_node_p> parser::list() {
  ...
  while (next != Token::done) {
    if (next == Token::empty_line) {
      blocks.push_back(std::make_shared<parse_node>(
          Token::block, std::vector<parse_node_p>{
                            std::make_shared<parse_node>(Token::empty_line)}));
...
parse_node_p parser::block() {
  /*
   * Order defined in standard as N -> G -> dimensions -> I/J/K -> feed function
   * -> spindle speed -> T -> M We parse blocks with the assumption that they
   * follow the most important parts of that definition: N -> G -> parameters ->
  ...
  if (param.parameter.defaults[next] == param_prg_name) {
    match(next);
    rowItem->appendChild(std::make_shared<parse_node>(
        Token::prg_name, std::vector<parse_node_p>{expr()}));
  }
  if (next == param.subsystem.symbol) {
    if (next == Token::g_word) {
      ...
      if (param.g.contains(g_use_subsystem_a) &&
        ...
        match(Token::g_word);
        rowItem->appendChild(
            std::make_shared<parse_node>(Token::subsystem_select, 1));
        ...
        match(Token::g_word);
        rowItem->appendChild(
            std::make_shared<parse_node>(Token::subsystem_select, 3));
      } else if (param.g.contains(g_use_subsystem_d) &&
                 std::get<int>(param.g[g_use_subsystem_d].word) == code) {
        match(Token::g_word);
        rowItem->appendChild(
            std::make_shared<parse_node>(Token::subsystem_select, 4));
      }
    } else {
      match(next);
      if (next == Token::num_literal) {
        ...
        rowItem->appendChild(
            std::make_shared<parse_node>(Token::subsystem_select, num));
      }
    }
  }
  if (next == Token::multi_letter) {
    ...
    if (param.functions.block.contains(ml)) {
      ...
      rowItem->appendChild(std::make_shared<parse_node>(
          Token::block_function, block_definition.type, blocks));
    } else {
      match(Token::multi_letter);
    }
  }
  ...
  while (next == Token::m_word) {
    ...
    if (active_m_numbers.contains(code)) {
      rowItem->appendChild(std::make_shared<parse_node>(
          Token::m_word, active_m_numbers.at(code)));
    }
  }
  ...
  if (next == Token::newline) {
     ...
     * We will not parse them, but will preserve them as a data node, for
     * debugging purposes. It is also possible that valid data ends up here, if
     * the order of operations is wrong. I.e. M-codes are written before
     * G-codes.
     */
    m_lexer->finish_line();
    if (next == Token::error || next == Token::unknown_function ||
    ...
    rowItem->appendChild(std::make_shared<parse_node>(Token::left_over_data,
...
parse_node_p parser::assignment() {
  ...
  if (next == Token::equals) {
    match(Token::equals);
    return std::make_shared<parse_node>(Token::equals,
                                        std::vector<parse_node_p>{var, expr()});
  }
  return var;
}
...
parse_node_p parser::number() {
  /*
   * TODO: We need to handle other ways of writing numbers that are valid, i.e.
   * floating point numbers defined with fixed period position. These will just
   * be parsed as int right now.
   */
  ...
  if (first_part.size() && first_part.at(0) == '0') {
    ...
    match(Token::period);
    if (next != Token::num_literal)
      return std::make_shared<parse_node>(Token::num_int,
                                          std::stoi(first_part));
    std::string second_part = m_lexer->string_value();
    match(Token::num_literal);
  ...
  if (active_g_numbers.contains(code)) {
    ...
    while (def.parameters.contains(next)) {
      ...
      nodes.push_back(std::make_shared<parse_node>(
          Token::parameter, param_type, std::vector<parse_node_p>{e}));
    }
  }
  return nodes;
}
...
parse_node_p parser::moreterms(parse_node_p left) {
  ...
  if (ops.contains(next)) {
    int op = next;
    match(op);
    auto p = std::make_shared<parse_node>(
        op, std::vector<parse_node_p>{left, term()});
    return moreterms(p);
  } else {
    return left;
  }
...
parse_node_p parser::morefactors(parse_node_p left) {
  ...
  if (ops.contains(next)) {
    ...
    match(op);
    auto p = std::make_shared<parse_node>(
        op, std::vector<parse_node_p>{left, factor()});
    return morefactors(p);
  } else {
    return left;
  }
...
parse_node_p parser::func() {
  if (next == Token::plus || next == Token::minus) {
    ...
    return morefuncs(
        std::make_shared<parse_node>(Token::string, m_lexer->string_value()));
    ...
    if (param.functions.unary.contains(t)) {
      match(Token::multi_letter);
      return morefuncs(std::make_shared<parse_node>(
          Token::unary_function, param.functions.unary[t],
          std::vector<parse_node_p>{expr()}));
    }
  ...
  if (m_lexer->hasValue()) {
    err = {error::Parsing, m_lexer->line(), m_lexer->pos(),
           "Expression non-parseable. Token: " + std::to_string(next) +
               " Variable: " + m_lexer->string_value()};
  } else {
    err = {error::Parsing, m_lexer->line(), m_lexer->pos(),
           "Expression non-parseable. Token: " + std::to_string(next) +
               " Variable: N/A"};
...
parse_node_p parser::morefuncs(parse_node_p left) {
  if (next == Token::multi_letter) {
    ...
    if (param.functions.binary.contains(t)) {
      match(Token::multi_letter);
      return std::make_shared<parse_node>(
          Token::binary_functions, param.functions.binary[t],
          std::vector<parse_node_p>{left, expr()});
    }
  }
  return left;
}
