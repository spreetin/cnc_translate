...
public:
  ...
  parse_node(int code, int value, std::vector<parse_node_p> children = {})
      : m_code(code), i_value(value), m_children(children) {
    value_type = Int;
  }
  parse_node(int code, double value, std::vector<parse_node_p> children = {})
      : m_code(code), d_value(value), m_children(children) {
    value_type = Double;
  }
  parse_node(int code, std::string value,
             std::vector<parse_node_p> children = {})
      : m_code(code), s_value(value), m_children(children) {
    value_type = String;
  }
  ...
  // ==== CHILDREN ====
  inline void setChildren(std::vector<parse_node_p> v) { m_children = v; }
  inline void prependChild(parse_node_p c) {
    m_children.insert(m_children.cbegin(), c);
  }
  inline void appendChild(parse_node_p c) { m_children.push_back(c); }
  ...
  [[nodiscard]] inline int childCount() const { return m_children.size(); }
  ...
  inline const int valueType() { return value_type; }
  [[nodiscard]] inline bool hasValue() const { return value_type != None; }

  // ==== TYPE ====
  [[nodiscard]] inline int type() const { return m_code; }
  inline void setType(int type) { m_code = type; }
...
protected:
  ...
  double d_value;
  std::string s_value;
  enum { None = -1, Int = 0, Double = 1, String = 2 } value_type = None;
