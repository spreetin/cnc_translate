 ...
 */
std::pair<std::string, parse_node_gen::machine_state>
parse_node_gen::generate_internal() {
 ...
 */
std::pair<std::string, parse_node_gen::machine_state>
parse_node_gen::generate_text() const {
#ifdef TESTING
  if (root->type() == Token::list) {
  ...
  if (!root->childCount() ||
      (root->type() != Token::block && root->type() != Token::list)) {
  ...
  for (auto &n : root->children()) {
    ...
    case Token::empty_line:
      // Just ignore this and let the block end. Token is just placeholder to
      // stop the block from being purged.
      break;
    case Token::queueing:
      if (param->queueing.has_queueing &&
          param->queueing.queueing_single_char) {
        ss << param->queueing.queueing_single_char;
      } else if (param->queueing.has_queueing &&
                 param->queueing.queueing_string.size()) {
        ss << param->queueing.queueing_string;
      } else {
        std::cerr << (error{error::Generating, -1, -1,
                            "No queueing enabled. Ignoring."})
                         .to_string();
      }
      break;
    case Token::queueing_num:
      if (param->queueing.has_queueing &&
          param->queueing.queueing_numbered_single_char) {
        ss << param->queueing.queueing_numbered_single_char << n->stringValue();
      } else if (param->queueing.has_queueing &&
                 param->queueing.queueing_numbered_string.size()) {
    ...
    case Token::g_word: {
      ...
      if (param->g.contains(g_code)) {
        ...
        switch (vIndex) {
        case 0:
          ss << std::to_string(std::get<int>(g_def.word)) << " ";
          break;
        case 1:
          ...
          break;
        case 2:
          ss << std::get<std::string>(g_def.word) << " ";
          break;
        }
      } else {
        std::cerr << (error{error::Generating, -1, -1,
                            "G-code not handled: " + std::to_string(g_code)})
                         .to_string();
      }
    ...
    case Token::m_word:
      if (param->m.contains(n->intValue())) {
        ...
        std::cerr << (error{error::Generating, -1, -1,
                            "No translation found for M-code with ID: " +
                                n->stringValue()})
                         .to_string();
      }
      break;
      ...
      if (param->subsystem.symbol == Token::g_word) {
        ...
        case 1:
          if (param->g.contains(g_use_subsystem_a)) {
            ss << "G"
               << std::to_string(
                      std::get<int>(param->g[g_use_subsystem_a].word));
          }
          break;
        case 2:
          if (param->g.contains(g_use_subsystem_b)) {
            ss << "G"
               << std::to_string(
                      std::get<int>(param->g[g_use_subsystem_b].word));
          }
          break;
        case 3:
          if (param->g.contains(g_use_subsystem_c)) {
            ss << "G"
               << std::to_string(
                      std::get<int>(param->g[g_use_subsystem_c].word));
          }
          break;
        case 4:
          if (param->g.contains(g_use_subsystem_d)) {
            ss << "G"
               << std::to_string(
                      std::get<int>(param->g[g_use_subsystem_d].word));
          }
          break;
        }
      } else {
    ...
    case Token::parameter: {
      ...
      if (!m) {
        std::cerr << (error{error::Generating, -1, -1,
                            "Can't match parameter with value " +
                                std::to_string(id) + " on G with code " +
                                std::to_string(g_code)})
                         .to_string();
        break;
      }
      ss << m;
    ...
    case Token::comment:
      if (param->comments.use_parenthesis) {
        ...
        std::cerr << (error{error::Generating, -1, -1,
                            "No comment format enabled, comment ignored."})
                         .to_string();
      }
      break;
    case Token::block:
...
/*!
 * @brief Does transformations on the tree adapting it to the chosen output
 * machine.
 *
 * Processes the tree, starting from the current root node for the object and
 * transforms it as needed for the chosen output machine. The original parsing
 * tree is not touched, instead new tree nodes are created as need when the
 * parse tree needs to be edited.
 *
 * Existing nodes or data members are reused when possible though, so deleting
 * any part of the original parse tree while an instance of this class exists
 * referencing the same tree can cause errors, crashes or faulty output.
 */
void parse_node_gen::transformTree() {
  ...
  // Check for unneccesary setting and resetting of the same function, like
  // setting G90/G91 when we already are in that state
  ...
  for (int i = 0; i < lastRoot.size(); i++) {
    ...
    std::erase_if(childList, [this]<typename T>(const T &p) {
      if (p->type() == Token::g_word && p->intValue() == g_absolute_dimension &&
          transform_state.dimension_mode ==
              machine_state::AbsoluteCoordinates) {
        return true;
      } else if (p->type() == Token::g_word &&
                 p->intValue() == g_incremental_dimension &&
                 transform_state.dimension_mode ==
                     machine_state::IncrementalCoordinates) {
        return true;
      }
      return false;
    });
    for (const auto &n1 : childList) {
      if (n1->type() == Token::g_word &&
          n1->intValue() == g_absolute_dimension) {
        transform_state.dimension_mode = machine_state::AbsoluteCoordinates;
      } else if (n1->type() == Token::g_word &&
                 n1->intValue() == g_incremental_dimension) {
  ...
  // Remove pair of G-codes that contradict each other (e.g. G90/G91 right after
  // each other)
  ...
  for (int i = 0; i < newRoot.size(); i++) {
    ...
    if (n->childCount() == 1 && n1->childCount() == 1 && n->getChild(0) &&
        n1->getChild(0) && n->getChild(0)->type() == Token::g_word &&
...
std::vector<parse_node_p>
parse_node_gen::squashTree(std::vector<parse_node_p> nodes) {
...
/*!
 * \brief Recursively remove nested block lists, creating a flat list of blocks
 * in the root node
 ...
 *
 * Expressions are such parts of code that doesn't have any side effects,
 * instead just becomes a (usually) numeric value when processed by the machine.
 * This includes numeric literals, variable references, numeric operators and
 * built-in functions that operate on numeric values, such as round, square root
...
std::string parse_node_gen::expr(const parse_node_p &n) const {
  if (!n)
    return "";
  std::stringstream ss;
  static const auto get_func =
      [](const int v, const std::map<std::string, int> *m) -> std::string {
    for (auto &i : *m) {
      if (i.second == v) {
        return i.first;
      }
  ...
  case Token::minus:
  case Token::slash:
    print_square(n, (char)n->type());
    break;
  case Token::binary_functions:
    print_square(n, get_func(n->intValue(), &(param->functions.binary)));
    break;
  // Values
  ...
  case Token::string:
    ...
    break;
  case Token::num_int:
    ss << std::to_string(n->intValue());
    break;
  case Token::num_float:
    // TODO Implement better float handling, to fit different machine parsing
    // requirements
    ss << std::to_string(n->doubleValue());
    break;
  case Token::variable: {
  ...
  case Token::unary_function:
    ss << get_func(n->intValue(), &(param->functions.unary)) << "["
       << expr(n->getChild(0)) << "]";
    break;
  // Unclear
 ...
 *
 * This function will first check for some common issues (like incremental
 * dimenstions instead of incremental mode when the machine doesn't support
 * those). If those are found it will fix for them and return, so we can take
 * another pass.
 *
 * If no such general issue is founc, a dedicated translation function for each
 * defined G-code is called, if such a function has been created.
 *
 * The default behaviour if the function can't find any defined transformations
 * is to return false (no transformation done), even if the code provided
 * wouldn't parse on the target machine. It isn't a compatibility checker, just
 * a best attempt to mangle code into compatibility.
 *
...
bool parse_node_gen::g(int code) {
  ...
  case g_linear_interpolation:
    return g_linear_interpolation_func();
  case g_linear_interpolation_with_collision_stop:
    return g_linear_interpolation_with_collision_stop_func();
  case g_linear_interpolation_with_feed_reduction:
    return g_linear_interpolation_with_feed_reduction_func();
  ...
  case g_rapid_positioning:
    return g_rapid_positioning_func();
 ...
 *
 * This function will simply call a dedicated translation function for each
 * defined M-code, if such a function has been created, or in applicable cases a
 * general transformation function for codes that can be handled in a general
 * way.
 *
 * The default behaviour if the function can't find any defined transformations
 * is to return false (no transformation done), even if the code provided
...
/*!
 * \brief Checks for unsupported coordinate specification modes and tries to fix
 * them
 *
 * Specifically what this function looks for are incremental coordinates
 * specified by parameter words while the machine is in absolute dimension mode,
 * and the machine doesn't support incremental word parameters.
 *
 * Also circle definitions using either circle center coordinates or a radius,
 * when the machine doesn't support this mode of specifying curves. They will
 * then be converted to the other type of parameter.
 *
...
bool parse_node_gen::transform_coordinates(int code) {
  // TODO Handle X/Y/Z coordinates while in incremental mode if machine has
  // U/W/V
  ...
  for (auto &n : nodes) {
    if (n->type() == Token::parameter) {
      ...
      bool found = false;
      for (auto &m :
           {param->g[code].parameter_types, param->parameter.defaults}) {
        for (auto &p : m) {
  ...
  for (const auto &n : nodes) {
      ...
      if (n->intValue() == param_circle_radius) {
        try {
          ...
          for (const auto &n2 : nodes) {
            ...
            if (n2->type() == Token::parameter && n2->intValue() == param_x &&
                n2->childCount()) {
              x2 = n2->getChild(0)->doubleValue();
            } else if (n2->type() == Token::parameter &&
                       n2->intValue() == param_z && n2->childCount()) {
              z2 = n2->getChild(0)->doubleValue();
            } else if (n2->type() == Token::parameter &&
                       n2->intValue() == param_incremental_x &&
                       n2->childCount()) {
              x2 = n2->getChild(0)->doubleValue() + x1;
            } else if (n2->type() == Token::parameter &&
                       n2->intValue() == param_incremental_z &&
                       n2->childCount()) {
              z2 = n2->getChild(0)->doubleValue() + z1;
            } else if (n2->type() == Token::parameter &&
                       n2->intValue() == param_circle_radius &&
                       n2->childCount()) {
    ...
    if (coord_to_radius) {
      ...
      for (auto &n2 : nodes) {
        if (n2->type() == Token::parameter &&
            n2->intValue() == param_circle_center_x && n2->childCount()) {
          xc = n2->getChild(0)->doubleValue();
        } else if (n2->type() == Token::parameter &&
                   n2->intValue() == param_circle_center_z &&
                   n2->childCount()) {
          zc = n->getChild(0)->doubleValue();
        } else if (n2->type() == Token::parameter &&
                   n2->intValue() == param_circle_center_incr_x &&
                   n2->childCount()) {
  ...
  for (const auto &n : nodes) {
    if (incremental_to_absolute && n->type() == Token::parameter) {
      switch (n->intValue()) {
      case param_incremental_x:
        appendChild(param_x, n->children());
        continue;
      case param_incremental_y:
        appendChild(param_y, n->children());
        continue;
      case param_incremental_z:
        appendChild(param_z, n->children());
        continue;
      case param_circle_center_incr_x:
        appendChild(param_circle_center_x, n->children());
        continue;
      case param_circle_center_incr_z:
        appendChild(param_circle_center_z, n->children());
        continue;
      }
    }
    if (coord_to_radius && n->type() == Token::parameter) {
      ...
      case param_circle_center_incr_z:
        if (radius) {
          appendChild(param_circle_radius,
                      {std::make_shared<parse_node>(Token::num_float, radius)});
          radius = 0;
        }
        continue;
      }
    ...
    if (radius_to_coord && n->type() == Token::parameter) {
      switch (n->intValue()) {
      case param_circle_radius:
        appendChild(param_circle_center_x,
                    {std::make_shared<parse_node>(Token::num_float, xc)});
        appendChild(param_circle_center_z,
                    {std::make_shared<parse_node>(Token::num_float, zc)});
        continue;
      }
    }
    newBlock->appendChild(n);
  ...
  if (incremental_to_absolute &&
    ...
    firstBlock->appendChild(
        std::make_shared<parse_node>(Token::g_word, g_incremental_dimension));
    lastBlock->appendChild(
        std::make_shared<parse_node>(Token::g_word, g_absolute_dimension));
  }
...
/*!
 * \brief Checks for feed or spindle rotation rates being specified as parameter
 * word if unsupported
 *
 * If a feed rate or a spindle rotation rate has been supplied as a parameter
 * word to a prepatory word, e.g. "G1 X10 F20" or "G1 X10 S500", but the machine
 * doesn't support changing these settings in the form of parameter words then
 * the parameter is transformed to the relevant G code for setting the value,
  ...
  for (const auto &n : nodes) {
    if (n->type() == Token::parameter) {
      ...
      for (int type : {param_feed, param_rpm}) {
              ...
              {std::make_shared<parse_node>(
                   ...
                   std::vector<parse_node_p>{
                       ...
                       std::make_shared<parse_node>(
                           ...
                           std::vector<parse_node_p>{
                               std::make_shared<parse_node>(
                                   Token::num_int + T, std::get<T>(value))})}),
               newBlock});
        };
        int op;
        if (type == param_feed) {
        ...
        switch (value.index()) {
        case 0:
          addNodes.operator()<0>(type, op);
          break;
        case 1:
          addNodes.operator()<1>(type, op);
          break;
        case 2:
          addNodes.operator()<2>(type, op);
        }
        root = newRoot;
        return true;
      }
 ...
 *
 * Sets parameters such as coordinate input mode (absolute/incremental) and
 * feed/spindle rotation rate mode when these are changed by the supplied G or M
 * code.
 *
 * The function will not update current tool position or current subsystem. This
 * has to be handled at the point of parsing.
 *
 ...
 */
void parse_node_gen::updateState(machine_state *state, int gcode,
                                 int mcode) const {
  switch (gcode) { // Change machine state if needed
  case -1:
    break;
  case g_absolute_dimension:
    state->dimension_mode = machine_state::AbsoluteCoordinates;
    break;
  case g_incremental_dimension:
    state->dimension_mode = machine_state::IncrementalCoordinates;
    break;
  case g_revolutions_per_minute:
    state->rpm_mode = machine_state::RevolutionsPerMinute;
    break;
  case g_constant_surface_speed:
    state->rpm_mode = machine_state::ConstantSurfaceSpeed;
    break;
  case g_feed_per_minute:
    state->feed_mode = machine_state::FeedPerMinute;
    break;
  case g_feed_per_revolution:
    state->feed_mode = machine_state::FeedPerRevolution;
    break;
  }
  switch (mcode) {
  case -1:
...
/*!
 * \brief Testing and debugging function for printing the an parse tree before
 ...
 * \param tabs Used internally for adding tab depth, can be set to left adjust
 * the entire output
 * \param tab_size the amount to indent nodes relative to their parent. Values
 * less than 2 is treated as 2.
 */
void parse_node_gen::printTree(parse_node_p rootNode, std::ostream &stream,
                               uint tabs, uint tab_size) const {
  if (tab_size < 2)
    tab_size = 2;
  if (tabs > 0) {
    for ([[maybe_unused]] const uint i :
         std::ranges::views::iota((uint)1, tabs)) {
  ...
  if (rootNode && rootNode->hasValue()) {
    ...
    case 0: {
      ...
      if (rootNode->type() == Token::g_word) {
        std::string_view name =
            g_code_name((GCodesAvailable)rootNode->intValue());
        stream << "Type: "
               << (name.empty() ? std::to_string(rootNode->intValue()) : name);
      } else if (rootNode->type() == Token::m_word) {
        std::string_view name =
            m_code_name((MCodesAvailable)rootNode->intValue());
        stream << "Type: "
               << (name.empty() ? std::to_string(rootNode->intValue()) : name);
      } else if (rootNode->type() == Token::parameter) {
        std::string_view name =
            parameter_name((ParameterType)rootNode->intValue());
        stream << "Type: "
               << (name.empty() ? std::to_string(rootNode->intValue()) : name);
      } else {
    ...
    case 1:
      stream << "Float: " << rootNode->stringValue();
      break;
    case 2:
