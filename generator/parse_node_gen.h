...
/*!
 * \brief Takes a parse tree root node and generates code for the chosen machine
 * from it
 */
class parse_node_gen {
public:
  struct machine_state {
    std::map<char, double> axis_positions = {{'X', 0}, {'Y', 0}, {'Z', 0}};
    enum feed_mode {
      FeedPerMinute,
      FeedPerRevolution
  ...
  parse_node_gen(MachineParameters *param, const parse_node_p &block,
                 const machine_state &machineState)
      : param(param), root(block), state_base(machineState) {}

  inline std::string generate() { return generate_internal().first; }
  void printTree(parse_node_p rootNode, std::ostream &stream, uint tabs = 0,
                 uint tab_size = 4) const EXPECTS(tab_size >= 2);
