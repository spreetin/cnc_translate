...
TEST_F(ParserBasic, g_continuing){
    ...
    EXPECT_EQ(p[0]->intValue(), std::get<int>(param.g[linear_interpolation].word));
    EXPECT_EQ(p[1]->type(), Token::parameter);
    int xParam;
    if (param.g[linear_interpolation].parameter_types.contains('X')){
        xParam = param.g[linear_interpolation].parameter_types['X'];
    } else {
        xParam = param.parameter_defaults['X'];
    }
    int tParam;
    if (param.g[linear_interpolation].parameter_types.contains('T')){
        tParam = param.g[linear_interpolation].parameter_types['T'];
    } else {
        tParam = param.parameter_defaults['T'];
    }
    ...
    EXPECT_EQ(p[2]->getChild(0)->intValue(), 202);
    match(Token::newline);
    ...
    EXPECT_EQ(p[1]->type(), Token::parameter);
    int zParam;
    if (param.g[linear_interpolation].parameter_types.contains('Z')){
        zParam = param.g[linear_interpolation].parameter_types['Z'];
    } else {
        zParam = param.parameter_defaults['Z'];
    }
    EXPECT_EQ(p[1]->intValue(), xParam);
    EXPECT_EQ(p[1]->childCount(), 1);
    EXPECT_EQ(p[1]->getChild(0)->type(), Token::num_int);
...
TEST_F(ParserBasic, block){
    ...
    ASSERT_TRUE(p);
    EXPECT_EQ(p->type(), Token::block);
    ASSERT_EQ(p->childCount(), 3) << p->getChild(0)->type() << " "
                                  << p->getChild(1)->type() << " "
                                  << p->getChild(2)->type() << " "
                                  << p->getChild(3)->type() << " "
                                  << p->getChild(3)->stringValue();
